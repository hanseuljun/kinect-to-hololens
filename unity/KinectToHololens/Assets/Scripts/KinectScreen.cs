...
// A port of the following c++ code: https://github.com/OpenKinect/libfreenect2/blob/master/src/registration.cpp
// Converts intrinsics parameters into vertices and texture coordinates.
public class KinectScreen
{
    private const float DEPTH_Q = 0.01f;
    private const float COLOR_Q = 0.002199f;
    ...
    // A method equivalent to RegistrationImpl::distort() in registration.cpp of libfreenect2.
    private static Vector2 Distort(int mx, int my, KinectIrIntrinsics irCameraParams)
    {
        // See http://en.wikipedia.org/wiki/Distortion_(optics) for the underlying theoretical model.
